...
echo -e ${Yellow}
PS3="Select : "
select begin in "Help" "Network Adaptors List" "Network Adaptors with Aircrack-ng" "De/Activate Device Monitoring" "Monitor Device" "Nmap on Devices on Network" "Change MAC" "Network Status" "Make Payload" "Create (MSF) Listener" "Exit"
do
############################
...
echo -e " 10   | Create MSF Listener             | Creates Meterpreter Shell"
echo -e " 11   | exit                            | Close program ${Yellow}"
echo 
			;;
		...
		"Network Status"*)
			echo -e ${Cyan}
			read -p "Enter Device : " dev_info
			echo -e
			sudo iw dev $dev_info scan | grep "SSID"
			sudo iw dev $dev_info scan 
			echo -e ${Yellow}
			;;
